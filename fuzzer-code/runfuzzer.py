...
libfd=open("image.offset","wb+")
libfd.seek(0,0)
libfd.write("0x0000000000000000")
libfd.seek(0,0)
libfd_mm=mmap.mmap(libfd.fileno(),0)
...
def execute2(tfl,fl,is_initial):
    args=config.SUT % tfl
    if is_initial == 1:
      pargs=config.PINTNT2CMD[:]
    else:
	  ...
	  op2val = 22
        if config.ALLCMPOP == True:
            if mat.group(op1start) =='' and mat.group(op2start) !='':
                ...
                if tempoff[0] ==-1:
                    continue
                ofs,hexstr=extract_offsetStr(tempoff[0],mat.group(op1val),fsize)
		if ofs != -1000 and tempoff[1] == 1:
			hexstr = hexstr[::-1]
            elif mat.group(op2start) =='' and mat.group(op1start) !='':
                tempoff=get_non_empty(mat,op1start)#mat.group(5)
                if tempoff[0] ==-1:
                    continue
                ofs,hexstr=extract_offsetStr(tempoff[0],mat.group(op2val),fsize)
		if ofs != -1000 and tempoff[1] == 1:
			hexstr = hexstr[::-1]
            else:
                ofs,hexstr=(-1000,[])
...
def get_taint(dirin, is_initial=0):
    ...
    for fl in files:
        ...
        config.ANALYSIS_MAP[fl] = [{'INT8':offset_api.get_offset(1), 'INT16':offset_api.get_offset(2), 'INT32':offset_api.get_offset(4), 'INT64':offset_api.get_offset(8)}, offset_api.get_arrays(), offset_api.get_all_offsets(), offset_api.get_memchr_reward_arr(), offset_api.get_cmp_reward_arr()]#, offset_api.get_malloc_reward_arr()]
    ...
    while lp <2:
        try:
     	...
     	try:
        	...
        	gau.emptyDir(config.INPUTD)
    	try:
        	os.mkdir(config.BUGD)
    	except:
        	gau.emptyDir(config.BUGD)
   ...
   # gau.die("dry run over..")
	...
	SPECIALCHANGED = False # this is set when at least one new input is added to the config.SPECIAL folder.
	  ...
	  files=os.listdir(config.BUGD)
        ...
        tnow=datetime.now().isoformat().replace(":","-")
        stat.write("\t%d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %s\t %s\n"%(genran,min(fitscore),maxfit,avefit,mnlen,mxlen,avlen,len(config.SEENBB),appcov,allcov, config.NUMINPUTS, config.fname, tnow))
        stat.flush()
        os.fsync(stat.fileno())
