...
//        "of", "fun.out", "specify output file name for fun");
...
KNOB<string> KnobFileName(KNOB_MODE_WRITEONCE, "pintool",
...
static VOID TimeoutF(VOID * arg)
{
    ...
    std::string dest = "outd/hangs";
    char buffer [5000];
    sprintf (buffer, "cp %s %s", KnobFileName.Value().c_str(), dest.c_str());
    if(KnobFileName.Value() != "$"){	
        int ret = std::system(buffer);
        if(ret == 0){
            cout<<"[*] Copied input to hangs"<<endl;		
        }
    }
...
int main(int argc, char * argv[])
{
    ...
    //out = new ofstream(KnobOutputFile1.Value().c_str());
