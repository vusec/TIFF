...
LIBTOMONITOR= ''#'libpcap.so.0.8'#'libjpeg.so.9'#'libpcap.so.0.8'#'libjpeg.so.9'#'libtiff.so.5'
...
#set the path where bug inputs are created
BUGD=os.path.join(BASETMP,"bug/")
#BUGD="bug/"
...
MOSTCOMNLAST= 6
RANDOMCOMN= True#True#True # this is to skip setting most common values for a offset sometimes.
...
INTER=os.path.join(BASETMP,"inter/")
...
# dictionary to keep arrays,datatypes,rewards types with key=file_name value = [dict(key=1,value=offsetswithdatatypeINT8,key=2,value=INT16..), list()-->arrays with starting offsets and ending offset, list()-->rewards function]
ANALYSIS_MAP=dict()
